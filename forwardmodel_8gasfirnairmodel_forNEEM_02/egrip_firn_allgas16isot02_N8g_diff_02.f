...
c---------------------------------------------------------------------
c Modified by Jamie 2025-10-15
c version egrip 02
c read in density file and interpolate onto the correct grid
c
C---------------------------------------------------------------------
...
c At EGRIP aircontent 1.0d-004
...
c initialize site-specific parameters
      open (26,file='inputfiles/inputdata_EGRIP_01.txt'
     $          ,status='old')
...
c      read (26,*) critdepth1, first0, first1, first2, first3
c      read (26,*) critdepth2, second0, second1, second2
      read(26, '(A80)') zrhofile
      read(26,'(A80)') ofolder
      ...
      close(26)
	print*, DiffFile
        print*, "read in zrhofile", zrhofile
...
c       rhoclose = second0 + second1*critdepth2 + second2*critdepth2**2	
c Jamie might need to fix this part (2025-10-15)

        open(22, file=zrhofile,status='old' )
      read (22,*) zrho1,dummy,rho1
      read (22,*) zrho2,dummy,rho2
...
c     $     *critdepth2))
c calculate depths and densities in open firn column using quadratic fit to data:
c     if (z(i).lt.critdepth1) 
c     $  ro(i)=first0+first1*z(i)+first2*exp(first3*(critdepth1-z(i)))
c      if (z(i).ge.critdepth1) ro(i)=second0+second1*z(i)+second2*z(i)**2		
c      if (z(i).ge.critdepth2) ro(i) = rhoice-(rhoice-rhoclose)
c    $  *exp(-(z(i)-critdepth2)/(rhoice-rhoclose)*(second1+2*second2
c     $     *critdepth2))
...
c edit 2025-10-15
c calculate density for the diffusive zone only:
c       if(i.le.dimlevels) then 
 293     if(z(i).gt.zrho2) then
            zrho1=zrho2
            rho1=rho2
            read (22,*) zrho2,dummy,rho2
         end if
         if(z(i).gt.zrho2) goto 293
        ro(i)=rho1+(rho2-rho1)/(zrho2-zrho1)*(z(i)-zrho1)
c        print*, z(i), ro(i)
c      endif
...
c            ro(i) = rhoice-(rhoice-rhoclose)
c     $  *exp(-(z(i)-critdepth2)/(rhoice-rhoclose)*(second1+2*second2
c     $     *critdepth2))
c Jamie, Read in Density
 294     if(z(i).gt.zrho2) then
            zrho1=zrho2
            rho1=rho2
            read (22,*) zrho2,dummy,rho2
         end if
         if(z(i).gt.zrho2) goto 294
         ro(i)=rho1+(rho2-rho1)/(zrho2-zrho1)*(z(i)-zrho1)
c         print*, z(i), ro(i)
...
c     $  *exp(-(z(i)-critdepth2)/(rhoice-rhoclose)*(second1+2*second2
c     $     *critdepth2))
 295     if(z(i).gt.zrho2) then
            zrho1=zrho2
            rho1=rho2
            read (22,*) zrho2,dummy,rho2
         end if
         if(z(i).gt.zrho2) goto 295
        ro(i)=rho1+(rho2-rho1)/(zrho2-zrho1)*(z(i)-zrho1)
c        print*,z(i),ro(i)
...
c		rolayer(j) = second0 + second1*zl + second2*zl**2
c Jamie edit 2025-10-15
c calculate density for the lock in zone: 
       dro_num = (ro(j+1)-ro(j))/(z(jj+1)-z(jj))*(depth(j)-z(jj))
       rolayer(j)=ro(jj)+dro_num
c        print*,rolayer(j)	

c 	calculate the total porosity:		     
		stotal(j) = 1 - rolayer(j)/rhoice
                print*, z(j),rolayer(j)/rhoice
...
c         print*, sclosed(j),stotal(j), (1-831.2/rhoice)
c        print*,z(j),(stotal(j)/(1-831.2/rhoice))**(-7.6)
...
c        print*, dvbubblel(i), sclosed(i), stotal(i), rolayer(i)
	vbl(i) = dvbubblel(i) + vbl(i-1)*rolayer(i)/rolayer(i-1)
c        print*, z(i), vbl(i), dvbubblel(i), rolayer(i)
...
c --- subroutine interpv 
c interpolate input file to the grid
        subroutine interpv(z1, t1, N1, z, t, N)
c     interpolate input file to the grid
        integer N1, N, i, j
        double precision z1(*), t1(*), z(*), t(*)
        double precision za, zb, ta, tb, zi, ti
        ...
        do 100 i = 1, N
10              if (z1(j+1) .lt. z(i)) then
                if (j .ge. N1-1) then
                print*, 'reached eof j=', j, ' N1=', N1
c     keep j constant if we reach the end of array
                else
                j = j + 1
                go to 10
                endif
        endif
        t(i) = t1(j)+(t1(j+1)-t1(j))/(z1(j+1)-z1(j))*(z(i)-z1(j))
100             continue
        return
        end